...
We were also given a Makefile (see [here][link2]) to compile `gemm.c` with different libraries and double point precision. Also in this case we slightly modified the code to compile with floating point precision as well, to write the results to file and to compile the executables in the right folder.
